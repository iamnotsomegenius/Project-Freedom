...
async def get_listings(
    ...
    # Note: For Supabase, we'd need to implement text search differently
    # This is a placeholder that will work for exact matches
    if industry:
    ...
    if min_revenue is not None or max_revenue is not None:
        ...
        if max_revenue is not None:
            # If both are present, this will overwrite the min_revenue query
            # In the implementation we need to handle both together
    ...
    # Note: Implementing text search in Supabase requires different approach
    # For now, this is a placeholder that will work for exact matches
    if search:
        # Simplified for now - in a real implementation, you would use PostgreSQL full-text search
        # or use a combination of LIKE clauses
